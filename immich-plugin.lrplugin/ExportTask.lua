...
local function getEditedPhotosCacheIfNeeded(exportParams)
    if
        exportParams.originalFileMode ~= "edited"
        and exportParams.originalFileMode ~= "original_plus_jpeg_if_edited"
    then
        return nil
    end
        ...
        if primaryId then
            ...
            elseif exportParams.albumMode == "folder" then
                ...
                if folderAlbumId then
                    immich:addAssetToAlbum(folderAlbumId, primaryId)
                end
            end
                ...
                if not originalPath then
                    ...
                    if not id then
                        ...
                        if
                            ...
                            and StackManager.hasEdits(photo, editedPhotosCache)
                        then
                            if string.upper(exportParams.LR_format or "") == "ORIGINAL" then
                                table.insert(
                                    stackWarnings,
                                ...
                                if exportId then
                                    ...
                                    if not immich:createStack({ exportId, id }) then
                                        table.insert(
                                            stackWarnings,
                                            photo:getFormattedMetadata("fileName") .. ": failed to create stack"
                                        )
                                    end
                                else
                                    table.insert(
                                        stackWarnings,
                                        photo:getFormattedMetadata("fileName") .. ": failed to upload rendered export"
                                    )
                                end
                            end
                        end
                        ...
                        elseif exportParams.albumMode == "folder" then
                            local folderAlbumId =
                                immich:createOrGetAlbumFolderBased(photo:getFormattedMetadata("folderName"))
                            if folderAlbumId then
                                immich:addAssetToAlbum(folderAlbumId, primaryId)
                            end
                        end
...
local function finalizeExport(immich, exportParams, albumId, useAlbum, atLeastSomeSuccess, failures, stackWarnings)
    ...
    local anySuccess = atLeastSomeSuccess == true
        or (type(atLeastSomeSuccess) == "table" and atLeastSomeSuccess[1] == true)
...
function ExportTask.processRenderedPhotos(functionContext, exportContext)
    ...
    if not exportSession or not exportParams then
        return nil
    end
